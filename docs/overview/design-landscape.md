...
```mermaid
flowchart TD
    subgraph 1_战略与发现 ["1. 战略与发现 (Strategy & Discovery)"]
        A[商业战略与业务目标] --> B[用户研究与问题发现]
        B --> C[需求规格定义 PRD]
    end

    subgraph 2_体验架构_UX ["2. 体验架构 (UX Research & Architecture)"]
        C --> D[信息架构 IA 设计]
        D --> E[用户旅程图 & 交互线框图]
        E --> F[可用性测试与流程验证]
    end

    subgraph 3_界面视觉_UI ["3. 界面与系统 (UI & Design System)"]
        F --> G[设计系统 Tokens & 组件]
        G --> H[高保真界面与响应式排版]
        H --> I[微动效与交互原型]
    end

    subgraph 4_工程实现_DEV ["4. 工程协同与实现 (FE Engineering)"]
        I --> J[Design Tokens 自动化编译]
        J --> K[前端组件库映射与页面开发]
        K --> L[PR 预览环境 Staging]
    end

    subgraph 5_质量与度量_QA ["5. 质量验收与度量 (QA & Growth)"]
        L --> M[Design QA 走查与像素比对]
        M --> N[WCAG 2.2 无障碍合规拦截]
        N --> O[上线发布与数据埋点回流]
    end

    style 2_体验架构_UX fill:#FAF9F6,stroke:#C8401E,stroke-width:2px
    style 3_界面视觉_UI fill:#FAF9F6,stroke:#C8401E,stroke-width:2px
    style 4_工程实现_DEV fill:#FAF9F6,stroke:#52565C,stroke-width:1px
```
...
```
  ┌─────────────────────────────────────────────────────────────┐
  │ 1.0 时代 (2000-2012)：Photoshop 静态切图与瀑布流交付         │
  │   └── 痛点：产物是一堆死图片（PSD），前端肉眼猜尺寸，还原度靠运气 │
  ├─────────────────────────────────────────────────────────────┤
  │ 2.0 时代 (2012-2020)：Sketch + 蓝湖/Zeplin 标尺导出          │
  │   └── 进步：有了坐标和参数；痛点：设计组件与代码依然完全脱节     │
  ├─────────────────────────────────────────────────────────────┤
  │ 3.0 时代 (2020-至今)：Figma + Design Tokens + 现代设计工程化 │
  │   └── 突破：单一事实源（SSOT）、Token 跨端发布、Design QA 前置 │
  └─────────────────────────────────────────────────────────────┘
```
